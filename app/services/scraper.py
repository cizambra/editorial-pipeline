...
_logger = get_logger("editorial.scraper")
...
def fetch_articles(force_refresh: bool = False) -> List[Dict[str, Any]]:
    ...
    if not force_refresh and CACHE_FILE.exists():
        ...
        _logger.info(
            ...
            extra={"fields": {"article_count": len(cache["articles"]), "updated_at": cache["updated_at"]}},
        )
...
def refresh_index() -> List[Dict[str, Any]]:
    ...
    if not articles:
        _logger.warning("Substack API returned no articles; falling back to RSS")
    ...
    _logger.info("Indexed articles", extra={"fields": {"article_count": len(articles)}})
    return articles
...
def index_new_articles() -> Dict[str, Any]:
    ...
    _logger.info(
        "Indexed new articles",
        extra={"fields": {"added": len(new_articles), "total": len(merged)}},
    )
...
def _fetch_api_page(offset: int, limit: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        _logger.warning(
            "Substack API page fetch failed",
            extra={"fields": {"offset": offset, "limit": limit, "error": str(e)}},
        )
        return []
...
def _fetch_via_rss() -> List[Dict[str, Any]]:
    ...
    _logger.info("Fetching RSS feed", extra={"fields": {"rss_url": RSS_URL}})
