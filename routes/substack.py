...
... (repeat for each note)"""
        repurposed = False
        try:
            rep_response = claude.messages.create(
                model="claude-haiku-4-5-20251001",
                max_tokens=7000,
...
Use the same voice as the original. Each format must stand alone. Output only the four labeled sections."""
    try:
        ...
        def extract(label: str) -> str:
            m = re.search(rf"{label}\s*:\s*\n([\s\S]*?)(?=\n(?:LINKEDIN|THREADS|INSTAGRAM|SUBSTACK_NOTE)\s*:|$)", raw, re.IGNORECASE)
            return m.group(1).strip() if m else ""
...
async def test_substack_connection(request: Request = None):
    ...
    if request is not None:
        auth.require_admin(request)
    try:
        return substack_client.test_connection()
    except Exception as exc:
...
async def list_subscribers(q: str = "", activity: Optional[int] = None, interval: str = "", offset: int = 0, limit: int = 50):
    ...
    try:
        return storage.get_subscribers(search=q, activity=activity, interval=interval, offset=offset, limit=limit)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
